...
void WIND_SPEED() {
 ...
 Serial.print("Wind_Speedreading : ");
  Serial.print(windSpeedReading);
  Serial.println(" m/s");
...
void MODE_SWITCHING() {
  ...
  else if ((RPM_T1 < 3 || RPM_T2 < 3 || RPM_T3 < 3) && Wind_Speed > 3) {
    // Motor mode
    Serial.println("MOTOR_MODE");

    // Check if the motor mode duration has elapsed
    if (millis() - motorModeStartTime >= motorModeDuration) {
      // Reset motor mode pins to low after 20 seconds
      SM_1_2 = LOW;
      SM_2_2 = LOW;
      SM_3_2 = LOW;
    }
    else {
      // Keep motor mode pins high
      SM_1_2 = HIGH;
      SM_2_2 = HIGH;
      SM_3_2 = HIGH;
    }
    
    // If motor mode is being activated for the first time or after 20 seconds
    if (SM_1_2 == LOW && SM_2_2 == LOW && SM_3_2 == LOW) {
      // Update motor mode start time
      motorModeStartTime = millis();
    }
  }
  ...
  // Reset motor mode start time
    motorModeStartTime = 0;
